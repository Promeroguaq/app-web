...
<div class="relative h-[40vh] sm:h-[50vh] md:h-[60vh] lg:h-[75vh] min-h-[300px] md:min-h-[400px] lg:min-h-[600px] overflow-hidden rounded-[32px] mb-6 md:mb-8 w-full" style="background: linear-gradient(135deg, #0f2d1a 0%, #10b981 50%, #059669 100%);">
    ...
    <div class="absolute bottom-0 left-0 right-0 p-4 md:p-8 lg:p-16 text-white">
        ...
        <p class="text-xs md:text-sm lg:text-lg xl:text-xl opacity-90 max-w-full md:max-w-3xl mb-3 md:mb-6" style="text-shadow: 1px 1px 6px rgba(0,0,0,0.6);">
            Explora los lugares más increíbles de Colombia
        </p>
    </div>
</div>
...
<div class="bg-white/80 backdrop-blur-sm p-4 md:p-6 mb-6 md:mb-8 rounded-[20px] md:rounded-[32px] shadow-lg">
    ...
    <div id="filters-container" class="hidden md:block">
        <form action="{{ route('destinos') }}" method="GET" id="filter-form">
            <!-- Búsqueda principal -->
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4 mb-4 md:mb-6">
                ...
                <div class="relative">
                    ...
                    <select name="departamento" 
                        ...
                        <option value="{{ $depto }}" {{ ($departamento ?? '') == $depto ? 'selected' : '' }}>
                            {{ $depto }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-end gap-2 md:gap-3">
                    ...
                    <a href="{{ route('destinos') }}" class="px-3 py-2 md:px-6 md:py-3 bg-gray-500 text-white rounded-full font-semibold hover:bg-gray-600 transition-all text-xs md:text-sm whitespace-nowrap">
...
<div class="flex items-center justify-between mb-4 md:mb-6">
    ...
    <div class="text-[10px] md:text-sm text-gray-600" data-total="{{ $total ?? 0 }}">
        Mostrando {{ $items->count() ?? 0 }} de {{ $total ?? 0 }} municipios
    </div>
</div>

<!-- Grid de Municipios -->
<div id="destinos-grid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
    ...
    <div class="rounded-[20px] md:rounded-[32px] overflow-hidden bg-white shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 w-full">
        <a href="{{ route('destinos.show', $item->id) }}" class="block">
            <div class="relative h-40 sm:h-48 md:h-56 overflow-hidden w-full">
                ...
                <div class="absolute bottom-3 right-3 md:bottom-4 md:right-4 bg-black/50 backdrop-blur-sm px-2 py-1 md:px-3 md:py-1.5 rounded-full text-[10px] md:text-xs text-white z-10">
                    <i class="fas fa-map-marker-alt mr-1"></i> {{ $item->departamento_nombre }}
                </div>
            </div>
            <div class="p-3 md:p-5 bg-white">
                ...
                <p class="text-[10px] md:text-xs lg:text-sm text-gray-600 mb-2 md:mb-3 line-clamp-2">{{ Str::limit($item->descripcion ?? '', 100) }}</p>
    ...
    <div class="col-span-full bg-white/80 backdrop-blur-sm p-6 md:p-8 lg:p-12 text-center text-gray-500 rounded-[20px] md:rounded-[32px] shadow-lg">
        <i class="fas fa-map-marked-alt text-3xl md:text-4xl mb-3 md:mb-4 opacity-50"></i>
        <p class="text-xs md:text-sm lg:text-base lg:text-lg">
            @if($search || $departamento || $region)
            No encontramos municipios con esos filtros. Intenta buscar por nombre, departamento o región.
            @else
            No hay municipios registrados en este momento.
            @endif
        </p>
        @if($search || $departamento || $region)
        <a href="{{ route('destinos') }}" class="inline-block mt-4 px-4 py-2 md:px-6 md:py-3 bg-gradient-to-r from-[#1D4ED8] to-[#1E40AF] text-white rounded-full font-semibold text-xs md:text-sm hover:shadow-lg transition-all">
...
<div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-8">
    <div class="bg-white/90 backdrop-blur-sm p-3 md:p-6 text-center rounded-[20px] md:rounded-[32px] shadow-lg">
        <div class="text-2xl md:text-4xl mb-1 md:mb-2">🗺️</div>
        <div class="text-xl md:text-3xl lg:text-4xl font-bold text-[#1D4ED8] mb-1 md:mb-2">{{ count($departamentos ?? []) }}</div>
        <div class="text-[10px] md:text-sm text-gray-600 font-medium">Departamentos</div>
    </div>
    <div class="bg-white/90 backdrop-blur-sm p-3 md:p-6 text-center rounded-[20px] md:rounded-[32px] shadow-lg">
        <div class="text-2xl md:text-4xl mb-1 md:mb-2">🏛️</div>
        <div class="text-xl md:text-3xl lg:text-4xl font-bold text-[#1D4ED8] mb-1 md:mb-2">{{ $total ?? 0 }}</div>
        <div class="text-[10px] md:text-sm text-gray-600 font-medium">Municipios</div>
    </div>
    <div class="bg-white/90 backdrop-blur-sm p-3 md:p-6 text-center rounded-[20px] md:rounded-[32px] shadow-lg">
        <div class="text-2xl md:text-4xl mb-1 md:mb-2">🌿</div>
        <div class="text-xl md:text-3xl lg:text-4xl font-bold text-[#1D4ED8] mb-1 md:mb-2">{{ count($regiones ?? []) }}</div>
        <div class="text-[10px] md:text-sm text-gray-600 font-medium">Regiones</div>
    </div>
    <div class="bg-white/90 backdrop-blur-sm p-3 md:p-6 text-center rounded-[20px] md:rounded-[32px] shadow-lg">
        <div class="text-2xl md:text-4xl mb-1 md:mb-2">📍</div>
        <div class="text-xl md:text-3xl lg:text-4xl font-bold text-[#1D4ED8] mb-1 md:mb-2">{{ $items->count() ?? 0 }}</div>
        <div class="text-[10px] md:text-sm text-gray-600 font-medium">Mostrados</div>
    </div>
</div>
