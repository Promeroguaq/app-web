...
@section('title', $item->nombre ?? 'Detalle Parque Temático')
...
<div class="relative h-[50vh] sm:h-[60vh] md:h-[75vh] min-h-[400px] md:min-h-[600px] overflow-hidden rounded-[32px] mb-12">
    ...
    @else
        <div class="absolute inset-0 bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
    <div class="lg:col-span-2">
        <div class="bg-white/80 backdrop-blur-sm p-8 rounded-[32px] shadow-lg mb-8">
            <h2 class="font-display text-2xl md:text-3xl font-bold text-gray-900 mb-6">Sobre el Parque</h2>
    ...
    <div class="lg:col-span-1">
        <div class="bg-white/80 backdrop-blur-sm p-8 rounded-[32px] shadow-lg sticky top-8">
            ...
            @if($item->locality_region)
            <div class="flex items-center gap-3 mb-4">
                <i class="fas fa-compass text-pink-600 text-xl"></i>
                <p class="text-gray-700">Región: {{ $item->locality_region }}</p>
            </div>
            @endif
            <div class="bg-pink-50 p-6 rounded-2xl mb-6">
                <div class="text-center">
                    <i class="fas fa-ticket-alt text-pink-600 text-4xl mb-3"></i>
                    <p class="text-sm text-gray-600">Diversión Garantizada</p>
                </div>
            </div>
            <a href="{{ route('puntos-interes.parques-tematicos') }}" class="block w-full text-center px-6 py-3 bg-gradient-to-r from-pink-500 to-rose-600 text-white rounded-full font-semibold hover:shadow-lg transition-all">
                ← Volver a Parques Temáticos
            </a>
        </div>
    </div>
</div>
...
<div class="bg-white/80 backdrop-blur-sm p-12 text-center bg-gradient-to-r from-pink-500 to-rose-600 text-white rounded-[32px] shadow-lg">
    <h2 class="font-display text-3xl font-bold mb-4">Explora Más Diversión</h2>
    <p class="text-lg opacity-90 mb-8 max-w-2xl mx-auto">Descubre otros parques y destinos de entretenimiento en Colombia</p>
    <div class="flex flex-wrap gap-4 justify-center">
        <a href="{{ route('puntos-interes.parques-tematicos') }}" class="px-6 py-3 bg-white text-pink-600 rounded-full font-semibold hover:shadow-lg transition-all">
            🎢 Todos los Parques
        </a>
