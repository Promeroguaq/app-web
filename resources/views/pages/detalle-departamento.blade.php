...
<x-detail.detail-hero
    ...
    :image="$item->imagen"
    :badge="'🇨🇴 Departamento'"
    :location="$item->region ? 'Región: ' . $item->region : null"
...
@php
    ...
    if(isset($item->region) && $item->region) {
        $infoItems[] = ['label' => 'Región', 'value' => $item->region, 'icon' => 'fa-globe-americas'];
    }
    if(isset($item->municipios) && count($item->municipios) > 0) {
        $infoItems[] = ['label' => 'Municipios', 'value' => count($item->allMunicipios) . ' municipios', 'icon' => 'fa-city'];
    }
@endphp
        ...
        <div class="rounded-[20px] md:rounded-[32px] overflow-hidden bg-white shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 w-full">
            <a href="{{ route('municipios.show.slugs', [$departamentoSlug, $municipio->slug]) }}" class="block">
                <div class="relative h-40 sm:h-48 md:h-56 overflow-hidden w-full">
                    ...
                    <div class="absolute bottom-3 right-3 md:bottom-4 md:right-4 bg-white/90 backdrop-blur-sm px-2 py-1 md:px-3 md:py-1.5 rounded-full text-[10px] md:text-xs text-gray-800 z-10">
                        <i class="fas fa-map-marker-alt text-emerald-600 mr-1"></i> {{ $item->nombre }}
                    </div>
                </div>
